...
if ($row['access'] == "Admin" ){
  echo "<script> window.location.href('printlist.php')</script>";
}
 else {
...
<div class="container" style="background-color: #d2f3f3; margin-top: 20px; width: auto; height: auto;" >
  ...
  <table width="100%"  >
        <tr>
          <th colspan="8" style="border-top-right-radius: 30px; border-top-left-radius: 30px;"><b><b>Welcome: <?php echo $_SESSION['ID']; ?>!<br></b>Print History:</b></th>
        </tr>
        <tr>
          <td><b><i>Row Num</i></b></td>
          <td><b><i>Date</i></b></td>
          <td><b><i>Creator ID</i></b></td>
          <td><b><i>Printed For</i></b></td>
        ...
        <?php
          ...
          if(isset($_POST['submit'])){
            ...
            $select=sqlsrv_query( $conn, "SELECT * FROM barcode where [Date] like '%".$search_file."%' or Propertyid like '%".$search_file."%' or Name like '%".$search_file."%' or [User] like '%".$search_file."%' ");
          
            }   
              else{
                 $select=sqlsrv_query( $conn, "SELECT * FROM barcode ");
                }
                $i=0;
               ...
               while($userrow=sqlsrv_fetch_array($select))  { $i++;
                ?>
                  ...
                  <td><b><?php echo $userrow['Name']  ?></b></td>
                  <td><b><?php echo $userrow['User'] ?></b></td>
                  <td><b><?php echo $userrow['Propertyid'] ?></b></td>   
              <?php
               }
                ?>
