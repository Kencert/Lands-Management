  ...
  <table width="100%"  >
        ...
        <?php
          if(isset($_POST['submit'])){
            ...
            $select=sqlsrv_query( $conn, "SELECT * FROM Allotment where  status = 'Approved' and Fname like '%".$search_file."%' or Lname like '%".$search_file."%'  or Propertyid like '%".$search_file."%'  or Title like '%".$search_file."%' ");
          
            }   
              else{
             $select=sqlsrv_query( $conn, "SELECT * FROM Allotment where status = 'Approved'");
          }
            $i=0;
        ...
        while($userrow=sqlsrv_fetch_array($select))  { $i++;
          ...
          <tr style="border-bottom-left-radius: 30px; border-bottom-right-radius: 30px;">
           ...
           echo "";
          if ($row['access'] == "Admin") {
            echo "<td><a href='print.php?id=".$userrow ['IdNum']."' >Print</a></td>";
           }
           ?>          
          <?php
      }
