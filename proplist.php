...
if ($row['access'] == "Admin" or $row['access'] == "Modify" or $row['access']== "Read"){
    echo "<script> window.location.href('proplist.php')</script>";  
    }
  else {
  ...
  <table width="100%"  >
        ...
        <?php
          if(isset($_POST['submit'])){
            ...
            $select=sqlsrv_query( $conn, "SELECT * FROM Property where Fname like '%".$search_file."%' or Lname like '%".$search_file."%'  or Email like '%".$search_file."%'  or Title like '%".$search_file."%' ");
          
            }   
            else{
              $select=sqlsrv_query( $conn, "SELECT * FROM Property ");
                }
              $i=0;
            ...
            if ($row['access'] == "Admin" or $row['access']=="Modify"){
                echo "<td><a href='propedit.php?id=".$userrow['Propertyid']."' >Edit</a></td>";
                }  
            if ($row['access'] == "Admin"){
                ?>
                <?php   
            if ($row['access'] == "Admin"){
              ...
              echo "<td><a href='fileupload.php?id=".$userrow['IdNum']."' >Upload Files</a></td>";
               }
               ?>
               <?php
                }}
