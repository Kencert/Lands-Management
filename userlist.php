...
<div class="container" style="background-color: #d2f3f3; margin-top: 20px; width: auto; height: auto;" >
  ...
  <table width="100%"  >
        ...
        <tr>
          ...
          <td><b><i>Last Log in time</i></b></td>
        ...
        <?php
         if(isset($_POST['submit'])){
            ...
            $select=sqlsrv_query( $conn, "SELECT * FROM Logins where Id_no like '%".$search_file."%' or uname like '%".$search_file."%'  or sname like '%".$search_file."%'  or user_mail like '%".$search_file."%' ");
          
            }   
              else{
          
          $select=sqlsrv_query( $conn, "SELECT * FROM Logins ");
          }
            $i=0;
        ...
        while($userrow=sqlsrv_fetch_array($select))  { $i++;
          ...
          <tr style="border-bottom-left-radius: 30px; border-bottom-right-radius: 30px;">
            ...
            <td><b><?php echo $userrow ['user_mail'] ?></b></td>
             <td><b><?php echo $userrow ['status'] ?></b></td>
                        <?php 
          if ($row['access'] == "Admin"){ ?>
            <td><b><?php echo $userrow ['access'] ?></b></td>
            <td><b><i><?php echo $userrow ['timel'] ?></i></b></td>
         <?php }?>

                        <?php 
            if ($row['access'] == "Admin"){
            echo "<td><a onClick = \"javascript: return confirm('Please confirm deletion');\" href='delete2.php?id=".$userrow['Id_no']."' >Delete</a></td>";
          
            echo "<td><a href='edit2.php?id=".$userrow['Id_no']."' >Edit</a></button></td>";
            }
           ?>
          <?php
      }
